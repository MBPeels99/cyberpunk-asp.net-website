    ...
    public partial class Profile : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["FullName"] != null)
            {
                ...
                navButtonsContainer.Visible = true;
                navButtonSignUp.Visible = false;
            }
            else
            {
                welcomeMessage.InnerHtml = "Welcome to Night City";
                navButtonsContainer.Visible = false;
                navButtonSignUp.Visible = true;
            }

            // Call the method to load and display the district
            LoadAndDisplayDistricts();

        }


        private void LoadAndDisplayDistricts()
        {
            // Fetch the district data from the database
            districts = GetDistrictsFromDatabase();

            // Bind the district data to the repeaters
            // TODO: Change to normal data binding
            ...
            // Check if a district ID is saved in the cookie and display corresponding district details
            ...
            {
                // Display details of the first district
                DisplayDistrictDetails(districts[0]);
            }
        }
        ...
        {
            // Ensure districtCookie is not null
            if (districtCookie == null)
            {
                districtCookie = new HttpCookie("District");
                districtCookie.Expires = DateTime.Now.AddDays(30); // Set an expiration date
            }
        ...
        public static string ConfirmDates(string startDate, string endDate)
        {
            try
            {
                ...
                {
                    return "District selection is empty.";
                }
